...
- 同日完成 M2 配套主线接入后, v2.1.0 (12) 最终候选在新租用三星 ARM64 16 KB 设备与 API 36 x86_64 16 KB 模拟器各通过完整公开 AIDL Release 冒烟; 两端宿主各 17 项功能测试, 新三星 Node NPM / conformance 13 项均通过. 此次新增 README 设备截图, 原生库字节未改变. 三星同时揭示宿主旧终端 AAR 的三个 4 KB ELF 仍触发系统兼容模式, 该宿主限制与插件原生 16 KB 验证分开记录. 原始结果, 最终五 APK 哈希, 宿主库清单与系统提示截图见 `benchmark/results/2026-09-10-mediainfo-main-integration.json`.

此前的三星租用设备 `localhost:31277` 存在并发安装活动: 插件独立验收卸载并确认插件包不存在后, 后续只读复核观察到 MediaInfo 2.0.0 (10) 与 Node Runtime 1.4.0 (152) 已安装. 再次只读复核时 MediaInfo 已变为 2.1.0 (11), 说明后续安装仍在进行. 这些在清理后出现的安装均予以保留; 验收记录分别保存即时清理确认和后续观察, 不将设备最终安装状态视为该轮 v2.1.0 运行状态.
