...
AutoJs6 MediaInfo 外掛為 AutoJs6 提供基於 MediaInfo 的媒體中繼資料讀取能力, 可產生完整報告, 查詢單一參數, 並輸出結構化 JSON 快照.
...
- 提供 `mediainfo` 外掛服務, 外掛 ID 為 `mediainfo`.
- 支援 AutoJs6 Node 環境中的 `mediainfo.read(path, options)` 和 `mediainfo.get(path, streamKind, parameter, options)`.
- 支援透過 `org.autojs.plugin.MEDIAINFO` 探索並呼叫外掛, 底層 AIDL 暴露 `inform`/`get`/`snapshot`.
- 內建 `arm64-v8a`/`armeabi-v7a`/`x86_64`/`x86` 的 `libmediainfo.so`.
...
路徑應為宿主允許存取的媒體檔案路徑, Node 環境可使用工作目錄內的相對路徑.
...
`mediainfo.get()` 的 `streamKind` 不區分大小寫, 會對應到 MediaInfo 原生串流類型.
...
- `includeInform`: 是否在快照中包含 MediaInfo 文字報告, 預設 `true`.
...
* [CHANGELOG.md](https://github.com/SuperMonster003/AutoJs6-Plugin-MediaInfo/blob/master/.changelog/CHANGELOG-zh-Hant-TW.md)
...
`strings.xml` 提供外掛描述和錯誤資訊在地化; `plugin_instruction.md` 提供宿主側展示的外掛使用說明. README 與 CHANGELOG 由 `.python/generate_markdown.py` 根據 JSON 原始檔生成.
...
- MediaInfo 官方專案: https://mediaarea.net/en/MediaInfo
- MediaInfoLib 官方專案: https://github.com/MediaArea/MediaInfoLib
- MediaInfoLib Android 專案: https://github.com/olegazyx/MediaInfoLib-android
