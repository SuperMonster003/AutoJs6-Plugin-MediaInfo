...
- `Исправление` Сборка выпусков проверяет версии и подписи текущих APK, исключая переименование старого APK в новую версию
...
Для архивирования выпуска запустите задачу `:app:appendDigestToReleasedFiles`, которая копирует APK из `app/release` в `app/releases` и переименовывает их по шаблону `autojs6-plugin-mediainfo-v2.1.3-<abi>-<crc32>.apk`.

Параметры сборки собраны в `version.properties`: минимальный SDK 24 (Android 7.0), целевой SDK 37, текущая версия 2.1.3.
